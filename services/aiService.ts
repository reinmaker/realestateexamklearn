...
async function generateFlashcardsOpenAI(documentContent?: string, count: number = 10): Promise<Flashcard[]> {
  ...
  const prompt = `אתה מורה מומחה למבחן התיווך הישראלי. תפקידך הוא לזהות את עקרונות הליבה המשפטיים, ההגדרות והכללים המרכזיים הנבחנים במסמך המצורף. בהתבסס על כך, צור סט חדש וייחודי של ${count} כרטיסיות לימוד בפורמט של שאלה-תשובה פתוחה (לא מבחן אמריקאי). חשוב: נסה לבחור מושגים שונים ונושאים שונים מאלו שכבר נבחרו בעבור. כל כרטיסייה צריכה להתמקד במושג אחד חשוב.

**חשוב מאוד - חל איסור מוחלט על שאלות מסוג מבחן אמריקאי:**
- לעולם אל תנסח שאלות כמו "איזה מהבאים נכון?", "מה נכון ביותר?", "איזה מהבאים נכון ביותר?"
- לעולם אל תכלול אפשרויות תשובה מרובות בשאלה
- לעולם אל תשאל שאלות שמתייחסות ל"אפשרויות" או "אופציות"

**השאלות חייבות להיות שאלות פתוחות בלבד:**
- דוגמאות טובות: "מהו חוזה פיתוח?", "מהי הערת אזהרה?", "מהו התהליך לרישום הערת אזהרה?", "מהי זכות זיקת הנאה?", "מהו הרוב הדרוש לאישור הצמדת חניה?"
- דוגמאות רעות (אסור!): "ימית חכרה דירה. איזה מהבאים נכון?", "מה נכון ביותר לגבי דמי תיווך?", "איזה מהבאים נכון ביותר?"

התשובות חייבות להיות קצרות ותמציתיות באופן קיצוני - משפט קצר אחד או שניים לכל היותר. יש לנסח אותן בשפה פשוטה וישירה. חל איסור מוחלט להשתמש בעיצוב טקסט כלשהו, במיוחד לא בהדגשה (ללא כוכביות **). אל תהפוך את שאלות המבחן הקיימות לכרטיסיות; במקום זאת, זקק מהן את הידע המשפטי הבסיסי. כל התוכן חייב להיות בעברית.
...
async function generateFlashcardsGemini(documentContent?: string, count: number = 10): Promise<Flashcard[]> {
  ...
  const prompt = `אתה מורה מומחה למבחן התיווך הישראלי. תפקידך הוא לזהות את עקרונות הליבה המשפטיים, ההגדרות והכללים המרכזיים הנבחנים במסמך המצורף. בהתבסס על כך, צור סט של ${count} כרטיסיות לימוד בפורמט של שאלה-תשובה פתוחה (לא מבחן אמריקאי). כל כרטיסייה צריכה להתמקד במושג אחד חשוב.
...
export interface AnswerAccuracyResult {
  accuracyScore: number; // 0-100
  feedback: string; // Brief feedback about the answer
  isCorrect: boolean; // Whether the answer is essentially correct (score >= 70)
}
...
export async function checkFlashcardAnswer(
  ...
  try {
    ...
    const systemPrompt = `אתה מורה מקצועי לבדיקת תשובות של תלמידים. תפקידך לבדוק את התשובה של התלמיד מול התשובה הנכונה ולתת ציון דיוק מ-0 עד 100.

כללים לבדיקה:
1. אם התשובה נכונה לחלוטין או מכילה את כל המידע הנדרש - ציון 90-100
2. אם התשובה נכונה ברובה אבל חסר מידע חשוב או יש טעויות קלות - ציון 70-89
3. אם התשובה חלקית נכונה אבל יש טעויות משמעותיות - ציון 40-69
4. אם התשובה שגויה ברובה או לא רלוונטית - ציון 0-39

החזר תשובה בפורמט JSON בלבד:
{
  "accuracyScore": <מספר מ-0 עד 100>,
  "feedback": "<משוב קצר בעברית, עד 100 תווים>",
  "isCorrect": <true אם הציון >= 70, אחרת false>
}`;
...
בדוק את תשובת התלמיד וציין את רמת הדיוק.`;
    ...
    const result = JSON.parse(responseText) as AnswerAccuracyResult;
    
    // Validate and clamp accuracy score
    result.accuracyScore = Math.max(0, Math.min(100, result.accuracyScore || 0));
    result.isCorrect = result.accuracyScore >= 70;
    
    return result;
  } catch (error) {
    console.error('Error checking flashcard answer:', error);
    // Fallback: simple text comparison
    const userLower = userAnswer.toLowerCase().trim();
    const correctLower = correctAnswer.toLowerCase().trim();
    const similarity = userLower === correctLower ? 100 : 
                      userLower.includes(correctLower) || correctLower.includes(userLower) ? 60 : 30;
    
    return {
      accuracyScore: similarity,
      feedback: similarity >= 70 ? 'תשובה טובה!' : similarity >= 40 ? 'קרוב, אבל חסר מידע' : 'נסה שוב',
...
צור ${count} כרטיסיות לימוד שמתמקדות בנושאים אלה. כל כרטיסייה צריכה להיות בפורמט שאלה-תשובה פתוחה (לא מבחן אמריקאי).
...
**השאלות חייבות להיות שאלות פתוחות בלבד:**
- דוגמאות טובות: "מהו חוזה פיתוח?", "מהי הערת אזהרה?", "מהו התהליך לרישום הערת אזהרה?", "מהי זכות זיקת הנאה?"
- דוגמאות רעות (אסור!): "ימית חכרה דירה. איזה מהבאים נכון?", "מה נכון ביותר לגבי דמי תיווך?"

התשובה צריכה להיות קצרה מאוד (משפט אחד או שניים). התשובות חייבות להיות פשוטות וישירות. חל איסור מוחלט להשתמש בעיצוב טקסט כלשהו (ללא **). כל התוכן בעברית.
...
צור ${count} כרטיסיות לימוד שמתמקדות בנושאים אלה. כל כרטיסייה צריכה להיות בפורמט שאלה-תשובה פתוחה (לא מבחן אמריקאי). השאלות חייבות להיות שאלות פתוחות וברורות - לעולם אל תנסח שאלות כמבחן אמריקאי עם אפשרויות תשובה מרובות. השתמש בשאלות פתוחות כמו "מהו...?", "מהי...?", "מהו התהליך...?" וכו'. התשובה צריכה להיות קצרה מאוד (משפט אחד או שניים). התשובות חייבות להיות פשוטות וישירות. חל איסור מוחלט להשתמש בעיצוב טקסט כלשהו (ללא **). כל התוכן בעברית.
